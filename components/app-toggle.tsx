"use client"

import { useId } from "react"
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group"
import { LogIn, LogOut } from "lucide-react"

type AppToggleProps = {
  value: "off" | "on";
  onValueChange: (value: "off" | "on") => void;
};

export default function AppToggle({ value, onValueChange }: AppToggleProps) {
  const id = useId();

  return (
    <div className="bg-input/50 inline-flex h-8 rounded-md p-0.5">
      <RadioGroup
        value={value}
        onValueChange={onValueChange}
        className="group after:bg-background has-focus-visible:after:border-ring has-focus-visible:after:ring-ring/50 relative inline-grid grid-cols-[1fr_1fr] items-center gap-0 text-sm font-medium after:absolute after:inset-y-0 after:w-1/2 after:rounded-sm after:shadow-xs after:transition-[translate,box-shadow] after:duration-300 after:ease-[cubic-bezier(0.16,1,0.3,1)] has-focus-visible:after:ring-[3px] data-[state=off]:after:translate-x-0 data-[state=on]:after:translate-x-full"
        data-state={value}
      >
        <label className="group-data-[state=on]:text-muted-foreground/70 relative gap-1 z-10 inline-flex h-full min-w-8 cursor-pointer items-center justify-center px-3 whitespace-nowrap transition-colors select-none">
          <LogIn className="size-4"/>
          <span className="hidden sm:inline">Sign-in</span>
          <RadioGroupItem id={`${id}-1`} value="off" className="sr-only" />
        </label>
        <label className="group-data-[state=off]:text-muted-foreground/70 relative gap-1 z-10 inline-flex h-full min-w-8 cursor-pointer items-center justify-center px-3 whitespace-nowrap transition-colors select-none">
          <LogOut className="size-4"/>
          <span className="hidden sm:inline">Sign-out</span>
          <RadioGroupItem id={`${id}-2`} value="on" className="sr-only" />
        </label>
      </RadioGroup>
    </div>
  )
}
