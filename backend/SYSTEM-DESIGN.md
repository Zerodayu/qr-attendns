...
| Method | Path                       | Auth | Role | Description                                                       |
| ------ | -------------------------- | ---- | ---- | ----------------------------------------------------------------- |
| POST   | /subscription/create-invoice | Yes  | any  | Create Xendit invoice. Body: `{ plan }`. Returns `{ invoiceUrl }`.|
| POST   | /subscription/webhook      | No   | none | Xendit webhook for invoice.paid / invoice.expired events.         |
...
| Method                                                   | Description                                                            |
| -------------------------------------------------------- | ---------------------------------------------------------------------- |
| `createInvoice(userId, plan, payerEmail?)`                | Creates a Xendit invoice, upserts subscription row with `pending` status. |
| `handleInvoicePaid(invoiceId)`                           | Activates subscription, sets `plan` on user, records period start/end. |
| `handleInvoiceExpired(invoiceId)`                        | Marks subscription as `expired`.                                       |
...
| `verifyWebhookToken(headers)`                            | Validates `x-callback-token` header against `XENDIT_WEBHOOK_TOKEN`.    |
...
| `XENDIT_SECRET_API_KEY`| Xendit secret API key for invoice creation|
